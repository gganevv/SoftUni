...
public abstract class DbContext
{
    ...
    internal static readonly Type[] AlloledSqlTypes =
    {
        typeof(string),
        typeof(int),
        typeof(uint),
    ...
    {
        object[] dbsSets = this.dbSetProperties
            .Select(dbSetInfo => dbSetInfo.Value.GetValue(this))
            .ToArray();

        foreach (IEnumerable<object> dbSet in dbsSets)
        {
        ...
        using (new ConnectionManager(this.connection))
        {
            ...
            foreach (IEnumerable dbSet in dbsSets)
            {
    ...
    private string[] GetEntityColumnNames(Type entityType)
    {
        ...
        string[] columnsTaken = entityType.GetProperties()
            ...
            DbContext.AlloledSqlTypes.Contains(pi.PropertyType))
            .Select(pi => pi.Name)
            .ToArray();
